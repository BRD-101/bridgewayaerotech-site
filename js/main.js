...
(function() {
  ...
  // Landing shows the featured weekly editorial plus the top four news articles
  // by relevance score, in a 2x2 grid. Section and nav links stay hidden until
  // content exists.
  var INSIGHT_SECTIONS = { MRO: 'MRO & Maintenance', OPS_FINANCE: 'Operations & Finance', AI_TECH: 'AI & Technology' };
  var TONE_LABELS = { AGREE: 'Favorable', CAUTION: 'Caution', CONCERN: 'Concern' };
  ...
  function renderFeaturedEditorial(post) {
    var box = document.getElementById('insightsFeatured');
    if (!box || !post) return;

    var label = document.createElement('p');
    label.className = 'editorial-label';
    label.textContent = 'The Bridgeway Position' + (TONE_LABELS[post.tone] ? '  ·  ' + TONE_LABELS[post.tone] : '');
    ...
    excerpt.className = 'editorial-excerpt';
    excerpt.textContent = post.summary || '';

    var byline = document.createElement('p');
    byline.className = 'editorial-byline';
    byline.textContent = (post.author || 'Brian R. Dillion') + ', ' + (post.author_role || 'Founder & Principal') +
      '  ·  ' + insightDateLabel(post.published_at);

    var link = document.createElement('a');
    link.className = 'insight-read';
    link.href = 'article.html?id=' + encodeURIComponent(post.id);
    link.textContent = 'Read the Column  →';
  ...
  function renderInsightCards(posts, count) {
    var grid = document.getElementById('insightsGrid');
    if (!grid || !posts.length) return;

    posts.slice(0, count || 4).forEach(function(post, i) {
      var card = document.createElement('article');
      card.className = 'insight-card reveal' + (i % 2 > 0 ? ' reveal-delay-' + (i % 2) : '');
  ...
  fetch('assets/blog/posts.json')
    ...
    .then(function(data) {
      ...
      if (!posts.length && !data.featured) return; // nothing yet — section stays hidden
      renderFeaturedEditorial(data.featured);
      renderInsightCards(posts, data.landing_count);
